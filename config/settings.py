...
# ブラウザから受け付ける元ファイルの上限。10MB超は圧縮する一方、極端に大きい通信は防ぐ。
MAX_RECEIVED_UPLOAD_BYTES = 30 * 1024 * 1024
MAX_IMAGE_PIXELS = 16_000_000
# 回数制限は全 Cloud Run インスタンスで共有するため、Cloud SQL のテーブルで管理する。
RATE_LIMIT_LOGIN_ATTEMPTS = 5
RATE_LIMIT_LOGIN_WINDOW_SECONDS = 15 * 60
RATE_LIMIT_SIGNUP_ATTEMPTS = 5
RATE_LIMIT_SIGNUP_WINDOW_SECONDS = 60 * 60
RATE_LIMIT_UPLOAD_ATTEMPTS = 10
RATE_LIMIT_UPLOAD_WINDOW_SECONDS = 60 * 60
...
# DEBUG=False でも例外のスタックトレースを Cloud Run の標準エラーへ出力する。
# ブラウザには詳細を返さず、Cloud Logging だけで安全に障害を調査できる。
LOGGING = {
    "version": 1,
